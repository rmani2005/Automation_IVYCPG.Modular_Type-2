...
public class TM_TGM_TC_02_ToverifyEDIT extends TestTaxManagementSuiteBase
{
			...
			String periodto_Month,String TaxGroup)
	{
		...
		try{
			...
			WebElement data=TestBase.getElement(driver, TestBase.OR.getProperty("TGM_search_Data"));
			if(data != null)
			{
				data.click();
			...
			WebElement EDITAPPEND=TestBase.getElement(driver, TestBase.OR.getProperty("TGM_EDIT"));
			if(EDITAPPEND != null)
			{
				EDITAPPEND.click();
				APP_LOGS.info("Clicked on EDITAPPEND label");
			}
			else
			{
				APP_LOGS.error("EDITAPPEND label is not available");
			}
			
			driver.manage().timeouts().implicitlyWait(100, TimeUnit.SECONDS);
			
			 WebElement TGM_Finish_Button1=TestBase.getElement(driver, TestBase.OR.getProperty("TGM_Finish_Button"));
				
				if(TGM_Finish_Button1.isDisplayed())
				{
					TGM_Finish_Button1.click();
					APP_LOGS.info("Clicked on the Finish button");
				}
				else
				{
				...
				Assert.assertEquals(actual1, "Details updated successfully!");
