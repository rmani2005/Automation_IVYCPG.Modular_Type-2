...
public class ToVerifySearchScreen_PL_LL_TC_11 extends TestStoreManagementSuiteBase
{
	...
	public void test1()
	{
		...
		try{
			...
			TestBase.getElement(driver, TestBase.OR.getProperty("seacrh_Label")).click();
			APP_LOGS.info("Clicked on Search Label");
			
			driver.manage().timeouts().implicitlyWait(100, TimeUnit.SECONDS);
			
			boolean location_code_text=TestBase.getElement(driver, TestBase.OR.getProperty("location_code_text")).isDisplayed();
            if(location_code_text)
            {
            	APP_LOGS.info("Location code field available");
            }
            else
            {
            	APP_LOGS.info("Location code field not available");
            }
            boolean location_name_text=TestBase.getElement(driver, TestBase.OR.getProperty("location_name_text")).isDisplayed();
            if(location_name_text)
            {
            	APP_LOGS.info("Location name field available");
            }
            else
            {
            	APP_LOGS.info("Location name field not available");
            }
			
            boolean retailer_code_text=TestBase.getElement(driver, TestBase.OR.getProperty("retailer_code_text")).isDisplayed();
			if(retailer_code_text)
			{
				APP_LOGS.info("Reatiler code Field available");
			}
			else
			{
				APP_LOGS.info("Reatiler code Field not available");
			}
			boolean retailer_name_text=TestBase.getElement(driver, TestBase.OR.getProperty("retailer_name_text")).isDisplayed();
			if(retailer_name_text)
			{
				APP_LOGS.info("Retailer Name Field available");
			}
			else
			{
				APP_LOGS.info("Retailer Name Field not available");
			}
