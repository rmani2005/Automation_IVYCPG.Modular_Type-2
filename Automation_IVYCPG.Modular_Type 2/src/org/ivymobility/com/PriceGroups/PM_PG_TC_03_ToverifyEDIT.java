...
public class PM_PG_TC_03_ToverifyEDIT extends TestPriceGroupsSuiteBase
{
	...
	public void test1(String code_Value,String name_Value,String price_Value,String editing_Value)
	{
		count++;
		...
		try{
			...
			WebElement edit_search_field=TestBase.getElement(driver, TestBase.OR.getProperty("PG_EDIT_SEARCH"));
			if(edit_search_field != null)
			{
				edit_search_field.sendKeys(code_Value);
				APP_LOGS.info("Code is searching");
			}
			else
			{
				APP_LOGS.info("Code search field is not available");
			}
			
			WebElement edit_value=TestBase.getElement(driver, TestBase.OR.getProperty("PG_EDIT_VALUE"));
			if(edit_value != null)
			{
				edit_value.click();
				APP_LOGS.info("Clicked on the data");
			}
			else
			{
				APP_LOGS.info("data is not available");
			}
			
			driver.manage().timeouts().implicitlyWait(400, TimeUnit.SECONDS);
			
			WebElement edit_label=TestBase.getElement(driver, TestBase.OR.getProperty("PG_EDIT_LABEL"));
			if(edit_label.isDisplayed())
			{
				edit_label.click();
				APP_LOGS.info("Clicked on the EDIT label");
			}
			else
			{
			...
			WebElement name_Field=TestBase.getElement(driver, TestBase.OR.getProperty("PG_name_Field"));
			if(name_Field.isDisplayed())
			{
				name_Field.clear();
				name_Field.sendKeys(editing_Value);
				APP_LOGS.info("editing_Value is sended to name field");
			}
			else
			{
				APP_LOGS.info("Name_Field is not available");
			}
			
			
			WebElement saveandproceed_EDIT=TestBase.getElement(driver, TestBase.OR.getProperty("PG_Save&Proceed_Button"));
			if(saveandproceed_EDIT != null)
			{
				saveandproceed_EDIT.click();
				APP_LOGS.info("Clicked on SAVE & PROCEED");
			}
			else
			{
			...
			WebElement finish_EDIT=TestBase.getElement(driver, TestBase.OR.getProperty("PG_ADD_Finish"));
			...
			String actual_EDIT=TestBase.getElement(driver, TestBase.OR.getProperty("POPUP_CONTENT")).getText();
			
			WebElement popup_button_EDIT=TestBase.getElement(driver, TestBase.OR.getProperty("POPUP_BUTTON"));
			
			if(popup_button_EDIT != null)
			{
				popup_button_EDIT.click();
				APP_LOGS.info("Clicked on ok  button in popup");
			}
			else
			{
			...
			Assert.assertEquals(actual_EDIT, "Saved Successfully");
