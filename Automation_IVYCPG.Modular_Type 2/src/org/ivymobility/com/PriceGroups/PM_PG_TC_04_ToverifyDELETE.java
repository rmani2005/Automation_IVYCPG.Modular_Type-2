...
public class PM_PG_TC_04_ToverifyDELETE extends TestPriceGroupsSuiteBase
{
	...
	public void test1(String code_Value,String name_Value,String price_Value)
	{
		...
		try{
			...
			TestBase.getElement(driver, TestBase.OR.getProperty("PG_SEARCH_FIELD")).sendKeys(code_Value);
			
			Thread.sleep(1000);

			WebElement delete_value=TestBase.getElement(driver, TestBase.OR.getProperty("PG_DELETE_VALUE"));
			if(delete_value.isDisplayed())
			{
				delete_value.click();
			...
			WebElement delete_label=TestBase.getElement(driver, TestBase.OR.getProperty("PG_DELETE_LABEL"));
			if(delete_label.isDisplayed())
			{
				delete_label.click();
				APP_LOGS.info("Clicked on the DELETE label");
			}
			else
			{
			...
			String actual_DELETE=TestBase.getElement(driver, TestBase.OR.getProperty("POPUP_CONTENT")).getText();
			
			WebElement popup_button_DELETE=TestBase.getElement(driver, TestBase.OR.getProperty("POPUP_BUTTON"));
			
			if(popup_button_DELETE != null)
			{
				popup_button_DELETE.click();
				APP_LOGS.info("Clicked on ok  button in popup");
			}
			else
			{
			...
			Assert.assertEquals(actual_DELETE, "details deleted successfully!");
