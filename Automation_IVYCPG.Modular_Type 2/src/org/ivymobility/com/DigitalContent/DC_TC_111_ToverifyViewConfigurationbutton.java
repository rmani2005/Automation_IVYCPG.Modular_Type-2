...
public class DC_TC_111_ToverifyViewConfigurationbutton extends TestDigitalContentSuitBase
{
	...
	public void test1()
	{
		...
		try{
			...
			WebElement viewConfiguration=TestBase.getElement(driver, TestBase.OR.getProperty("DC_DCM_View_Configuration_Label"));
			if(viewConfiguration.isDisplayed())
			{
				viewConfiguration.click();
				APP_LOGS.info("Clicked on ViewConfiguration label");
			}
			else
			{
				APP_LOGS.info("ViewConfiguration label is not available");
			}
			
			Thread.sleep(1000);
		
			WebElement viewConfiguration_Header=TestBase.getElement(driver, TestBase.OR.getProperty("DC_DCM_View_Configuration_Header"));
			if(viewConfiguration_Header.isDisplayed())
			{
				
				APP_LOGS.info("Displayed viewConfiguration_Header label");
			}
			else
			{
				APP_LOGS.info("Not displayed viewConfiguration_Header label");
			}
